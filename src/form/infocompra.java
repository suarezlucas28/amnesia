...
public class infocompra extends javax.swing.JPanel {
    ...
    public infocompra() throws ClassNotFoundException, InstantiationException, IllegalAccessException, SQLException {
        initComponents();
        cargacliente();
        des_op();
        des_btn();
    ...
    try {
    if (op1.isSelected()){
    sentencia = "SELECT `com_codigo`,`pro_nombre`,`com_fechor`,`com_total`,`com_estado` FROM `compras`,`proveedor`,usuarios WHERE `proveedor`.`pro_codigo` = `compras`.`pro_codigo` AND compras.usu_codigo = usuarios.usu_codigo AND com_estado LIKE 'ACTIVA' ORDER BY `com_codigo` DESC";    
    } else if (op2.isSelected()){
    sentencia = "SELECT `compras`.`com_codigo`,`pro_nombre`,`com_fechor`,`com_total`,`com_estado`,`compras_detalle`.`art_codigo`,`art_descri`,`com_cosuni`,`com_cantid`,`com_costot` FROM usuarios,`compras`,`proveedor`,`compras_detalle`,`articulo` WHERE compras.usu_codigo = usuarios.usu_codigo AND `proveedor`.`pro_codigo` = `compras`.`pro_codigo` AND `compras`.`com_codigo` = `compras_detalle`.`com_codigo` AND `compras_detalle`.`art_codigo` = `articulo`.`art_codigo` AND `compras`.`com_codigo` = "+txtcodigo.getText();
    } else if (op3.isSelected()){
    sentencia = "SELECT `com_codigo`,`pro_nombre`,`com_fechor`,`com_total`,`com_estado` FROM `compras`,`proveedor`,usuarios WHERE `proveedor`.`pro_codigo` = `compras`.`pro_codigo` AND compras.usu_codigo = usuarios.usu_codigo AND com_estado LIKE 'ACTIVA' AND LEFT(`com_fechor`,10) BETWEEN  '"+txtfdesde.getText()+"' AND '"+txtfhasta.getText()+"' ORDER BY `com_codigo` DESC";    
    } else if (op4.isSelected()){
    sentencia = "SELECT `com_codigo`,`pro_nombre`,`com_fechor`,`com_total`,`com_estado` FROM `compras`,`proveedor`,usuarios WHERE `proveedor`.`pro_codigo` = `compras`.`pro_codigo` AND compras.usu_codigo = usuarios.usu_codigo AND com_estado LIKE 'ACTIVA' AND `compras`.`pro_codigo` = "+cliente+" ORDER BY `com_codigo` DESC";    
    } else if (op8.isSelected()){
    sentencia = "SELECT `com_codigo`,`pro_nombre`,`com_fechor`,`com_total`,`com_estado` FROM `compras`,`proveedor`,usuarios WHERE `proveedor`.`pro_codigo` = `compras`.`pro_codigo` AND compras.usu_codigo = usuarios.usu_codigo AND com_estado LIKE 'ANULADA' ORDER BY `com_codigo` DESC";    
    }
