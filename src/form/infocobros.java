...
public class infocobros extends javax.swing.JPanel {
    ...
    String SQL = "SELECT `cli_nomape` FROM `cliente` where cli_nomape != 'SIN CLIENTE'";
    ...
    try {
    if (op1.isSelected()){
    sentencia = "SELECT `cc_test`.`cco_codigo` AS cobro_cob_codigo,`cc_test`.`cco_fecha` AS cobro_cob_horfec,cc_test.`cco_monent` AS cobro_cob_monto,UCASE(cliente.`cli_nomape`) AS cliente_nombre FROM `cc_test`,`cliente` cliente WHERE `cc_test`.`cli_codigo` = cliente.`cli_codigo` AND `cco_estado` LIKE 'ACTIVO' ORDER BY `cc_test`.`cco_codigo` DESC";        
    } else if (op2.isSelected()){        
    sentencia = "SELECT `cc_test`.`cco_codigo` AS cobro_cob_codigo,`cc_test`.`cco_fecha` AS cobro_cob_horfec,cc_test.`cco_monent` AS cobro_cob_monto,UCASE(cliente.`cli_nomape`) AS cliente_nombre FROM `cc_test`,`cliente` cliente WHERE `cc_test`.`cli_codigo` = cliente.`cli_codigo` AND `cco_estado` LIKE 'ACTIVO' and `cc_test`.`cco_codigo` = "+txtcodigo.getText();
    } else if (op3.isSelected()){
    sentencia = "SELECT `cc_test`.`cco_codigo` AS cobro_cob_codigo,`cc_test`.`cco_fecha` AS cobro_cob_horfec,cc_test.`cco_monent` AS cobro_cob_monto,UCASE(cliente.`cli_nomape`) AS cliente_nombre FROM `cc_test`,`cliente` cliente WHERE `cc_test`.`cli_codigo` = cliente.`cli_codigo` AND `cco_estado` LIKE 'ACTIVO' AND LEFT(`cco_fecha`,10) BETWEEN  '"+txtfdesde.getText()+"' AND '"+txtfhasta.getText()+"' order by cc_test.`cco_codigo` desc";    
    } else if (op4.isSelected()){
    sentencia = "SELECT `cc_test`.`cco_codigo` AS cobro_cob_codigo,`cc_test`.`cco_fecha` AS cobro_cob_horfec,cc_test.`cco_monent` AS cobro_cob_monto,UCASE(cliente.`cli_nomape`) AS cliente_nombre FROM `cc_test`,`cliente` cliente WHERE `cc_test`.`cli_codigo` = cliente.`cli_codigo` AND `cco_estado` LIKE 'ACTIVO' AND `cc_test`.`cli_codigo` = "+cliente+" ORDER BY cc_test.cco_codigo desc";    
    } else if (op8.isSelected()){
    sentencia = "SELECT `cc_test`.`cco_codigo` AS cobro_cob_codigo,`cc_test`.`cco_fecha` AS cobro_cob_horfec,cc_test.`cco_monent` AS cobro_cob_monto,UCASE(cliente.`cli_nomape`) AS cliente_nombre FROM `cc_test`,`cliente` cliente WHERE `cc_test`.`cli_codigo` = cliente.`cli_codigo` AND `cco_estado` LIKE 'ANULADO' order by cc_test.`cco_codigo` desc";    
    } 
    ...
    private void initComponents() {
        ...
        cbo_cliente.setEnabled(false);
        cbo_cliente.addKeyListener(new java.awt.event.KeyAdapter() {
            public void keyPressed(java.awt.event.KeyEvent evt) {
                cbo_clienteKeyPressed(evt);
            }
        });
...
op8.doClick();
    ...
    private void cbo_clienteKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_cbo_clienteKeyPressed
        if (evt.getKeyChar() == KeyEvent.VK_ENTER)
        {
            btngenerar.setEnabled(true);
            btngenerar.requestFocus();
        }        // TODO add your handling code here:
    }//GEN-LAST:event_cbo_clienteKeyPressed
